...
struct DetailsViewController1: View {
    ...
    //                            let uiImage: UIImage = image.asUIImage()
                        ...
                        Button {
                            ...
                            Image(systemName: "ellipsis")
                                .font(.title)
                        }.confirmationDialog("\(platform) Wallpaper", isPresented: $presentActionSheet, actions: {
                            Button {
    //                            let image: Image = Image(string)
    //                            let uiImage: UIImage = image.asUIImage()
                                if let completedUIImage = completedUIImage {
                ...
                ToolbarItem(placement: .navigationBarTrailing, content: {
                    if let completedImage = completedImage {
                        ShareLink(item: completedImage, preview: SharePreview("\(platform) Wallpaper", image: completedImage), label: {
                            HStack {
